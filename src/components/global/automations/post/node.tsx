...
const PostNode = ({ id }: PostNodeProps) => {
  ...
  return data?.data && data?.data?.posts?.length > 0 ? (
    <div className="w-10/12 lg:w-8/12 relative xl:w-4/12 p-5 rounded-xl flex flex-col bg-[#1D1D1D] gap-y-3">
      ...
      <div className="flex gap-x-2">
        <Warning />
        <p className="font-semibold text-lg">If they comment on...</p>
      </div>
      <div className="bg-background-80 p-3 rounded-xl flex flex-col gap-y-2">
        <div className="flex gap-x-2 items-center">
          <InstagramBlue />
          <div className="font-bold text-lg">These posts</div>
        </div>
        <div className="flex flex-wrap gap-x-2 mt-3">
          {data?.data?.posts?.map((post) => (
            ...
            >
              <Image fill sizes="100vw" src={post.media} alt="post" />
            </div>
          ))}
        </div>
      </div>
